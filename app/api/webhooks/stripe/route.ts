...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Processed Stripe webhook: ${result.type}`);
...
async function updateDatabaseFromWebhook(result: any, gateway: 'stripe' | 'razorpay') {
  ...
  if (result.type.includes('payment_succeeded') || result.type.includes('charged')) {
    // Find subscription
    ...
    if (subscription) {
      ...
      await prisma.payment.create({
        data: {
          ...
          gateway: 'stripe',
          region: 'global',
          [`${gateway}PaymentIntentId`]: data.paymentIntentId || data.paymentId,
        },
      });
    }
  }
