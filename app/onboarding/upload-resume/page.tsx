...
export default function UploadResumePage() {
  ...
  const { resumeData, updateResumeData } = useOnboardingStore();

  function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    const f = e.target.files?.[0];
    if (f) {
      updateResumeData({ fileName: f.name, wantsBuilder: false });
    }
  }

  function handleCreateResume() {
    updateResumeData({ wantsBuilder: true, fileName: null });
          ...
          <div className="mt-10 space-y-6">
            <label className="block cursor-pointer rounded-2xl border-2 border-dashed border-[#C8C6FF] bg-[#F9F8FF] p-8 hover:border-[#4D31EC] transition">
              ...
              <div className="flex flex-col items-center justify-center text-[#4D31EC]">
                ...
                {resumeData.fileName && (
                  <span className="mt-1 text-sm text-gray-500">{resumeData.fileName}</span>
                )}
              </div>
            </label>
